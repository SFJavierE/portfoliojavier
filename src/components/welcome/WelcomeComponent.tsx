...
const WelcomeComponent = () : React.ReactElement => {
    ...
    const isMobile: boolean = useMediaQuery({maxWidth: 900})
        ...
        return (
            <div>
                <div className="d-flex align-items-center justify-content-center">
                    {!seeMe && <button className={animationCoinOne ? "bg-cold-4 bg-penguin coin" : "bg-cold-4 bg-penguin"} style={{width: '80%', height: '30vh', borderRadius: '100%'}} onClick={()=>onTransition(true)}/>}
                    {seeMe && <button className={animationCoinTwo ? "bg-me coin" : "bg-me"} style={{width: '80%', height: '30vh', borderRadius: '100%'}} onClick={()=>onTransition(false)}/>}
                </div>
                <div className="pt-4 text-start" style={{marginBottom: '-20px'}}>
                    <div style={{marginBottom: '-10px'}}>
                        <span className="text-cold-3" style={{fontSize: '18px'}}>Javier Eduardo Sosa Fuch</span>
                        <hr className='border-cold-3' style={{marginTop: '-2px'}}/>
                    </div>
                    <span style={{fontSize: '16px'}}>Argentina, Bs. As., Mar del Plata</span>
                    <br />
                    <span style={{fontSize: '16px'}}>Nivel de Inglés Conversacional</span>
                    <br />
                    <span style={{fontSize: '16px'}}>Edad: {age}</span>
                </div>
